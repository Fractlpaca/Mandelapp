var elements={
	...
	pscreen: document.getElementById("pscreen"),
...
pscr=elements.pscreen.getContext("2d",{alpha:false});
...
var mstate={
	...
	clickX: 0,
	clickY:0,
	setparams: function(x=0,y=0,zoom=1,limit=600){
		input.set("xcor",x);
		input.set("ycor",y);
		input.set("zoom",zoom);
...
function createPreview(context=state.context){
...
var colorState={
	...
	setparams: function(){
		input.set("relCol",this.relCol?"on":"off");
		input.set("colorStep",this.colorStep);
		input.set("colorBegin",this.colorBegin);
		input.set("inSetColor",this.inSetColor);
		input.set("gradType",this.gradType);
		input.set("dir",this.dir);
		updateGrad("usergrad");
	},
	export: function(){
...
function updateGrad(action,event=undefined){
	...
	else if(action=="usergrad"){
		input.set("pointNum",0);
		color.num=colorState.gradSize;
		num=color.num;
		input.set("gradSize",num);
		input.set("presets","none");
		color.points=colorState.gradient;
		updateGradPos(0);
		input.element('slider').max=num-1;
	}
	colorState.gradient=color.points;
	colorState.export();
...
function run(){
	...
	document.getElementById("formx").value=mstate.xcor;
